...
export function renderText(lines, options)
{
    ...
    for (let row = 0; row < lines.length; row++)
    {
        ...
        {
            const texture = Resources.shared.getFrame('text_' + msg[n]);
    ...
    if (options && options.blackBG) {
        let bg = new PIXI.Sprite(Resources.shared.getFrame('black'));
...
export class HealthUI
{
    constructor()
    {
        ...
        this.hearts = [];
        this.fullHeart = Resources.shared.getFrame('full_bigheart');
        this.halfHeart = Resources.shared.getFrame('half_bigheart');
        this.emptyHeart = Resources.shared.getFrame('empty_bigheart');
...
export class ItemSlotUI
{
    constructor(item, args)
    {
        ...
        this.itemSprite = new PIXI.Sprite(
            Resources.shared.getFrame(item.image));
        this.itemSprite.anchor.set(0.5, 0);
        this.itemSprite.x = 0.5;
        this.itemSprite.y = 0;
        this.slotSprite = new PIXI.Sprite(Resources.shared.getFrame('small_slot'));
    ...
    setItem(item)
    {
        ...
        if (this.item !== item) {
            this.item = item;
            this.itemSprite.texture = Resources.shared.getFrame(item.image);
        }
    }
}
...
class Button
{
    constructor(stateList)
    {
        ...
        stateList.forEach(arg => {
            let name = arg[0];
            let img = arg[1];
            this.states[name] = Resources.shared.getFrame(img);
